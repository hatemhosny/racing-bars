...
export function renderInitalView(
  selector: string,
  dateSlice: Data[],
  renderOptions: any,
  getDateFn: () => string,
  ...
  function renderControls(element: HTMLElement, showControls: Options['showControls'], controlFns: any) {
    const icons = {
...
export function renderFrame(dateSlice: Data[], renderOptions: any, getDateFn: () => string) {
  if (!x) {
    return;
  }
...
export function resize(element: HTMLElement, renderOptions: any, resetFn: () => void) {
  if (
